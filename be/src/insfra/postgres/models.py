...
class ReactionModel(SQLModel, table=True):
    ...
    article_id: Optional[int] = Field(
        default=None, primary_key=True, foreign_key="article.id", index=True)
    type: str = Field(default=ReactionType.Favorite.value,
                      description='type', max_length=30)

    created_at: datetime.datetime = Field(
        description='Created At', default_factory=get_current)
    updated_at: datetime.datetime = Field(
        description='Updated At', default_factory=get_current)
...
class UserModel(SQLModel, table=True):
    ...
    id: Optional[int] = Field(
        primary_key=True, index=True, default_factory=generate_new_id)
    ...
    hashed_password: str = Field(
        description='hashed_password', max_length=255, default_factory=get_current)
    created_at: datetime.datetime = Field(
        description='Created At', default_factory=get_current)
    updated_at: datetime.datetime = Field(
        description='Updated At', default_factory=get_current)

    reacted_articles: List["ArticleModel"] = Relationship(
        back_populates="reacted_users", link_model=ReactionModel)
...
class ArticleModel(SQLModel, table=True):
    ...
    created_by: Optional[int] = Field(
        default=None, description='created_by', foreign_key="user.id", index=True)
    # created_user: Optional[UserModel] = Relationship(back_populates="articles")

    reacted_users: List["UserModel"] = Relationship(
        back_populates="reacted_articles", link_model=ReactionModel)
